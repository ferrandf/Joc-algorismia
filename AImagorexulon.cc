 ...
 */
#define PLAYER_NAME magorexulon
...
struct PLAYER_NAME : public Player {
  ...
  void inside_bfs(int i, int j, int d, queue<Pos>& q, VVE& M){
    if(i>=0 and i<60 and j>=0 and j<60 and M[i][j] == 0){
      M[i][j] = d;
      Pos p;
      p.i = i; p.j = j;
      q.push(p);
  ...
  void inside_tresors(P p, int i, int j, int d, queue<P>& pending, VVE& thematrix){
    if(i>=p.first-3 and i < p.first +3 and j >= p.second -3 and j < p.second+3 and cell(i,j).type != 2 and cell(i,j).type != 3 and not cell(i,j).treasure){
      if(cell(i,j).type == 4)thematrix[i][j] = min(thematrix[i][j], d+=cell(i,j).turns);
      else thematrix[i][j] = min(thematrix[i][j], d);
      P pos;
  ...
  void bfs_tresors(VVE& thematrix, VPB& tresors, VVC& G){
    for(int i = 0; i<tresors.size(); ++i){
      P p;
      p.first = tresors[i].first.first; p.second = tresors[i].first.second;
      thematrix[p.first][p.second] = 0;
      queue<P> pending;
      pending.push(tresors[i].first);
      cout << endl;
      cout << tresors[i].first.first << ' ' << tresors[i].first.second << endl;
      cout << endl;
      int d = 0;
      while(not pending.empty()){
        P q = pending.front();
        ...
        if(G[x][y] != 'T'){
          pending.pop();
          ++d;
          inside_tresors(p,x-1,y-1,d,pending,thematrix);
          inside_tresors(p,x-1,y,d,pending,thematrix);
          inside_tresors(p,x-1,y+1,d,pending,thematrix);
          inside_tresors(p,x,y-1,d,pending,thematrix);
          inside_tresors(p,x,y+1,d,pending,thematrix);
          inside_tresors(p,x+1,y-1,d,pending,thematrix);
          inside_tresors(p,x+1,y,d,pending,thematrix);
          inside_tresors(p,x+1,y+1,d,pending,thematrix);
  ...
  virtual void play () {
    ...
    VVE thematrix(60,VE(60, 1e9));
    if(round() == 0){
      bfs_tresors(thematrix, tresors, G);
    }
    ...
    for(int i = -1; i<61; ++i){
      for(int j = -1; j<61; ++j){
        if(i == -1 or i == 60) cout << abs((j)%10) << " ";
          ...
          if(G[i][j] == '.' and thematrix[i][j] != 1e9) cout << thematrix[i][j]%10 << " ";
          else cout << G[i][j] << " ";
        }
      }
      cout << endl;
